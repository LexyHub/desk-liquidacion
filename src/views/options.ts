export const SiONo = [
  { label: "Sí", value: "si" },
  { label: "No", value: "no" },
];

export const EstadoCivil = [
  { label: "Soltero/a", value: "soltero" },
  { label: "Conviviente Civil", value: "conviviente-civil" },
  { label: "Casado/a", value: "casado" },
  { label: "Divorciado/a", value: "divorciado" },
  { label: "Viudo/a", value: "viudo" },
  { label: "Separado/a", value: "separado" },
];

export const RegimenMarital = [
  { label: "Sociedad conyugal", value: "sociedad-conyugal" },
  { label: "Separación de bienes", value: "separacion-bienes" },
  {
    label: "Participación en los gananciales",
    value: "participacion-en-los-gananciales",
  },
];

export const TipoTrabajo = [
  { label: "Independiente", value: "independiente" },
  { label: "Dependiente", value: "dependiente" },
];

export const TipoTrabajador = [
  { label: "Privado", value: "privado" },
  { label: "Público", value: "publico" },
];
